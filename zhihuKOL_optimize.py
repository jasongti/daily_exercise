    ...
    for i in data['data']:
        if i['follower_count'] > 500000:
            if i['url_token'] not in to_crawl and i['url_token'] not in crawled:
                print i['name']
                result.append([i['name'].encode('utf8'), i['follower_count'], i['is_following']])
...
def get_kol(user):
    print 'crawling:', user
    url = url1 + user + url2 + '0'
    total = crawl(url)
    count = 0
    while count < total and count < 200:
        count += 20
        url = url1 + user + url2 + str(count)
        task = threading.Thread(target=crawl, args=(url,))
        task.start()
